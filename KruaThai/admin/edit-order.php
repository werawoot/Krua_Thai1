<?php
/**
 * Krua Thai - Edit Order (แก้ไข JavaScript Error แล้ว)
...
<body>
    ...
    <script>
        // ✅ แก้ไข JavaScript Error - ใช้ print-order.php แทน inline printing
        ...
        // ✅ แก้ไข printOrder function - ใช้ print-order.php
        function printOrder() {
            const orderId = '<?= $order["id"] ?>';
            
            // เปิด print-order.php ในแท็บใหม่พร้อม auto-print
            ...
            if (printWindow) {
                ...
                // Fallback หาก popup ถูกบล็อก
                alert('Please allow popups to print orders. Opening in new tab instead.');
        ...
        document.querySelectorAll('.form-control').forEach(input => {
            input.addEventListener('focus', function() {
                ...
                this.style.boxShadow = '0 0 0 3px rgba(207, 114, 58, 0.1)';
            });
        ...
        document.querySelectorAll('select[name="status"], select[name="kitchen_status"]').forEach(select => {
        ...
        // Real-time price calculation for new items
        document.addEventListener('change', function(e) {
            if (e.target.name === 'new_menu_id') {
                const selectedOption = e.target.options[e.target.selectedIndex];
                const price = selectedOption.dataset.price;
                ...
                if (price && quantityInput) {
                    const total = parseFloat(price) * parseInt(quantityInput.value || 1);
                    console.log(`Total for new item: ฿${total.toFixed(2)}`);
                    
                    // Optional: แสดงผลรวมใน UI
                    let priceDisplay = document.getElementById('newItemTotal');
                    if (!priceDisplay) {
                        priceDisplay = document.createElement('div');
                        priceDisplay.id = 'newItemTotal';
                        priceDisplay.style.cssText = 'margin-top: 0.5rem; font-weight: 600; color: var(--curry);';
                        quantityInput.parentElement.appendChild(priceDisplay);
                    }
                    priceDisplay.textContent = `Total: ฿${total.toFixed(2)}`;
                }
            }
            
            if (e.target.name === 'new_quantity') {
                const menuSelect = document.querySelector('select[name="new_menu_id"]');
                if (menuSelect.value) {
                    // Trigger calculation
        ...
        console.log('✅ Edit Order page loaded successfully - JavaScript errors fixed!');
    </script>
</body>
</html>